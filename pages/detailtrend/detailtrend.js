...
Page({
	...
	onLoad(options) {
		...
		if (this.data.trendId != null && this.data.trendId != '') {
			postParamsRequest("/trend/gettrend", {
					trendId: this.data.trendId
				})
				.then((value) => {
					const {data} = value;
	...
	clickDeleteComment: function () {
		...
		postParamsRequest("/comment/remove", {
				commentId: commentId
			})
			.then((value) => {
				const {code,msg} = value;
	...
	clickDeleteTrend: function () {
		...
		wx.showModal({
			...
			success: function (res) {
				if (res.confirm) {
					postParamsRequest("/trend/remove", {
							trendId: that.data.trend.trendId
						})
						.then((value) => {
							const {code,msg} = value;
							// console.log(value);
